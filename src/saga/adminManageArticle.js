...
export function* getAllArticleList(pageNum,rowsPerPage){
...
export function* getAllArticleListFlow(){
    while(true){
        ...
        let response = yield call(getAllArticleList,pageNum,rowsPerPage);
        if(response){
...
export function* dealArticleFlow(){
    while(true){
        ...
        const id = request.article._id;
        const state = request.article.state;
